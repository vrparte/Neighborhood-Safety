<script>
  ...
  // Add this section for Report Incident functionality
  let currentStep = 1;
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-red-50 rounded-xl p-6 shadow-lg">
      <h2 class="text-xl font-bold mb-4">Current Neighborhood</h2>
      <p class="text-lg mb-6">{currentLocation.neighborhood}</p>
      
      <div class="flex justify-center mb-8">
        <div class="bg-red-500 text-white px-8 py-3 rounded-full text-xl font-bold">
          CAUTION
        </div>
      </div>
  ...
  <div class="mt-8 bg-white rounded-xl p-6 shadow-lg">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      {#each recentIncidents as incident}
        <div class={`${incident.bgColor} rounded-lg p-4 shadow-md hover:shadow-lg transition-all duration-300`}>
  ...
  <div class="mt-8 bg-white rounded-xl p-6 shadow-lg">
    ...
    <div class="flex items-center justify-between mb-8">
      {#each ['Select Type', 'Location', 'Details'] as step, i}
